...
from tensorflow.keras.layers import Dense, LSTM, Flatten, ConvLSTM2D, Conv3D, MaxPooling3D, Dropout, GlobalAveragePooling2D, BatchNormalization,GlobalAveragePooling1D
...
# Video folder path
video_folder = "https://drive.google.com/drive/folders/1Q5NSEGnR3cgUm6S8WtKIg4mfS3-PO0bZ?usp=drive_link"
...
def build_transformer_model():
    inputs = Input(shape=(10, 2048))
    x = GlobalAveragePooling1D()(inputs)  # ✅ Fix input shape issue
    ...
    outputs = Dense(1, activation='sigmoid')(x)  # ✅ Corrected Output Layer
...
models = {
   ...
   # "Transformer": (build_transformer_model(), X_train, X_test) 
results = {}

for name, (model, X_train_input, X_test_input) in models.items():
    print(f"\n🔹 Training {name}...")
    model.fit(X_train_input, y_train, epochs=10, batch_size=16, validation_split=0.1, class_weight=class_weights, verbose=1)
